...
{
    public partial class ManualReceiptSecond : System.Web.UI.Page
    {
        DataSet ds = null;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                double Weight1st = _receiptRepo.GetWeight1st(txtTicketNo.Text.Trim());
                ...
                Tuple<bool, List<NetWeightModel>, NetReceiptTotalModel> resultTuple = _receiptRepo.UpdateReceipt(_WBTRXModel, _WBTRXGRADING2nd);
                ...
                if (result)
                {
                    ...
                    double potongan = stnet1 * cutpresent; //stnet1 * (myDate > myDateLest ? cutpresent : 0); 
                    double tRotasi = stnet1 * deduc;
                    Session["Potongan"] = potongan;
                    Session["SumDeducation"] = String.Format("{0,15:#,##0 ;(#,##0);0  }", tRotasi.ToString());
                    Session["Sum2ND"] = String.Format("{0,15:#,##0 ;(#,##0);0   }", Math.Floor(double.Parse(listNetWeight[0].WeightHeavy.ToString()) - double.Parse(listNetWeight[1].WeightHeavy.ToString()) - tRotasi - potongan));
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            HttpContext.Current.Items["potongan"] = Session["CutWeigh"];
