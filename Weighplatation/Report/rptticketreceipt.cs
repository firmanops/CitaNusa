...
{
    public partial class rptticketreceipt : DevExpress.XtraReports.UI.XtraReport
    {
        UserRepo userRepo = new UserRepo();
        public rptticketreceipt()
        {
        ...
        private void BindingData()
        {
            ...
            if (_contex.Items["Ticketls"] != null)
            {
                ...
                spotongan =   _contex.Items["potongan"].ToString();


                double deduct = (netWeightModels[0].WeightHeavy - netWeightModels[1].WeightHeavy) * double.Parse(_contex.Items["QtyDeducation"].ToString());
                double potongan = (netWeightModels[0].WeightHeavy - netWeightModels[1].WeightHeavy) * (double.Parse(spotongan) / 100);
                ...
                txtTotal.Text = String.Format("{0,15:#,##0 ;(#,##0);0   }",  Math.Floor((double.Parse(txtSubTotal.Text) -  potongan - deduct)));
